...
/* Note:
 * Currently only the CBI transport protocol has been implemented, and it
...
static unsigned int usb_get_max_lun(struct us_data *us)
{
	...
	len = usb_control_msg(us->pusb_dev,
			      ...
			      result, sizeof(char),
			      USB_CNTL_TIMEOUT * 5);
	//printf("Get Max LUN -> len = %i, result = %i\n", len, (int) *result);
...
static int usb_stor_probe_device(struct usb_device *udev)
{
	...
	for (lun = 0; lun <= max_lun && usb_max_devs < USB_MAX_STOR_DEV;
		...
		if (usb_stor_get_info(udev, &usb_stor[start],
			...
			//printf("partype: %d\n", blkdev->part_type);
			init_part(blkdev); // FIXME mntmn was part_init
			//printf("partype: %d\n", blkdev->part_type);
			usb_max_devs++;
			//printf("%s: Found device %p\n", __func__, udev);
		}
	}
...
{
	if (mode == 1)
		printf("[usb-storage] scanning usb for storage devices...\n");
	...
	printf("[usb-storage] %d storage device(s) found\n", usb_max_devs);
	if (usb_max_devs > 0)
		return 0;
	return -1;
}
		...
		do {
			...
			if (us->pusb_dev->status != 0) {
				...
				if (us->pusb_dev->status & USB_ST_STALLED) {
					printf("stalled ->clearing endpoint" \
					      "halt for pipe 0x%x\n", pipe);
					stat = us->pusb_dev->status;
					usb_clear_halt(us->pusb_dev, pipe);
					us->pusb_dev->status = stat;
					if (this_xfer == partial) {
						printf("bulk transferred" \
						      "with error %lX," \
						      " but data ok\n",
						      us->pusb_dev->status);
						return 0;
				...
				printf("bulk transferred with error");
				if (this_xfer == partial) {
...
static int usb_stor_CB_comdat(struct scsi_cmd *srb, struct us_data *us)
{
	...
	while (retry--) {
		printf("CBI gets a command: Try %d\n", 5 - retry);
#ifdef DEBUG
		usb_show_srb(srb);
#endif
		...
		if (result < 0) {
			if (us->pusb_dev->status & USB_ST_STALLED) {
				...
				printf(" stall during command found," \
				      " clear pipe\n");
				usb_clear_halt(us->pusb_dev,
					      usb_sndctrlpipe(us->pusb_dev, 0));
				us->pusb_dev->status = status;
			}
			printf(" error during command %02X" \
			      " Stat = %lX\n", srb->cmd[0],
			      us->pusb_dev->status);
			return result;
		}
...
do_retry:
	...
	printf("auto request returned %d\n", result);
	/* if this is an CBI Protocol, get IRQ */
	if (us->protocol == US_PR_CBI)
		       ...
		       unsigned long start, unsigned short blocks)
{
	...
	srb->cmdlen = 12;
	//printf("read10: start %lx blocks %x\n", start, blocks);
			...
			unsigned long start, unsigned short blocks)
{
	...
	srb->cmd[8] = (unsigned char) blocks & 0xff;
	srb->cmdlen = 12;
	//printf("write10: start %lx blocks %x\n", start, blocks);
	return ss->transport(srb, ss);
}
				   ...
				   lbaint_t blkcnt, void *buffer)
{
	...
	// FIXME debug
	//printf("usb_read: dev %d startblk " LBAF ", blccnt " LBAF " buffer %x\n",
...
retry_it:
	...
	//printf("usb_read: end startblk " LBAF ", blccnt %x buffer %x\n",
...
retry_it:
	...
	//printf("usb_write: end startblk " LBAF ", blccnt %x buffer %x\n",
	//      start, smallblks, buf_addr);
...
unsigned long usb_stor_read_direct(int dev_index, unsigned long blknr, unsigned long blkcnt, void *buffer) {
	return usb_stor_read(&usb_dev_desc[dev_index], blknr, blkcnt, buffer);
}

unsigned long usb_stor_write_direct(int dev_index, unsigned long blknr, unsigned long blkcnt, void *buffer) {
	return usb_stor_write(&usb_dev_desc[dev_index], blknr, blkcnt, buffer);
}

// returns capactity as number of blocks
unsigned long usb_stor_get_capacity(int dev_index) {
	return usb_dev_desc[dev_index].lba;
}
		      ...
		      struct us_data *ss)
{
	...
	if (usb_set_interface(dev, iface->desc.bInterfaceNumber, 0) ||
		...
		printf("[usb-storage] Problems with device\n");
		return 0;
	}
	...
	if (ss->subclass != US_SC_UFI && ss->subclass != US_SC_SCSI &&
		...
		printf("[usb-storage] Sorry, protocol %d not yet supported.\n", ss->subclass);
		return 0;
	}
	if (ss->ep_int) {
		      ...
		      struct blk_desc *dev_desc)
{
	...
	printf("[usb-storage] address %d\n", dev_desc->target);

	if (usb_inquiry(pccb, ss)) {
		printf("[usb-storage] %s: usb_inquiry() failed\n", __func__);
		return -1;
	}
	...
	if (((perq & 0x1f) == 0x1f) || ((perq & 0x1f) == 0x0d)) {
		printf("[usb-storage] %s: unknown/unsupported device\n", __func__);
		return 0;
	}
	if ((modi&0x80) == 0x80) {
		/* drive is removable */
...
#endif /* CONFIG_USB_BIN_FIXUP */
	printf("[usb-storage] ISO Vers %X, Response Data %X\n", usb_stor_buf[2],
	      usb_stor_buf[3]);
	if (usb_test_unit_ready(pccb, ss)) {
		printf("[usb-storage] Device NOT ready\n"
	...
	if (usb_read_capacity(pccb, ss) != 0) {
		printf("[usb-storage] READ_CAP ERROR\n");
		ss->flags &= ~USB_READY;
		cap[0] = 2880;
		cap[1] = 0x200;
	}
	//printf("Read Capacity returns: 0x%08lx, 0x%08lx\n", cap[0], cap[1]);
#if 0
	...
	printf("[usb-storage] Capacity = 0x%08lx, blocksz = 0x%08lx\n", capacity, blksz);
